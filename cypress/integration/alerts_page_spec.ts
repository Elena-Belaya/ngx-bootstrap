...
describe('Alerts page test suite', () => {
  ...
  describe('Basic alert', () => {
    ...
    it('success, info, warning and danger types of alerts are displayed', () => {
      ...
      cy.get(basicDemo).find('div').as('alertsBasic').eq(0)
        .should('be.visible')
        .and('to.have.class', alertTypes[0]);
      cy.get('@alertsBasic').eq(1)
        .should('be.visible')
        .and('to.have.class', alertTypes[1]);
      cy.get('@alertsBasic').eq(2)
        .should('be.visible')
        .and('to.have.class', alertTypes[2]);
      cy.get('@alertsBasic').eq(3)
        .should('be.visible')
        .and('to.have.class', alertTypes[3]);
    });
  });
